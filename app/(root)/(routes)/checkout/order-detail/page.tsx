...
const OrderDetailPage = () => {
  ...
  const fetchOrderDetail = async (code: string | null) => {
    try {
      setLoading(true)
      const accessToken: string = getUserToken()
      const res: {
        statusCode: number,
        message: string,
        data: {
      ...
      } = await getOrderDetail(code, accessToken)
              ...
              <div className="p-[24px] bg-white rounded-[12px]">
                ...
                <div className="flex flex-col gap-y-4 py-6 border-b-2 border-[#DFE3E8]">
                  ...
                  <div className="flex flex-row justify-between items-center">
                    ...
                    <h3 className="text-base font-bold text-[#000000]">
                      {formatToCurrencyVND(parseInt(detailOrder?.subTotal))}
                    </h3>
                  </div>
                  <div className="flex flex-row justify-between items-center">
                  ...
                  <div className="flex flex-row justify-between items-center">
                    <h3 className="text-base font-normal text-[#637381]">Phụ phí</h3>
                    <h3 className="text-base font-bold text-[#000000]">0VND</h3>
                  </div>
                </div>
                <div className="flex flex-col pt-6">
                  <div className="flex flex-row justify-between items-center">
                    ...
                    <h3 className="text-base font-bold text-[#000000]">
                      {formatToCurrencyVND(parseInt(detailOrder?.total))}
                    </h3>
                  </div>
                </div>
              </div>
              <div className="p-[24px] bg-white h-fit rounded-[12px]">
                ...
                <div className="flex flex-col gap-y-4 pt-4">
                  <div className="flex flex-row justify-between items-center">
                    ...
                    <h3 className="text-base font-bold text-[#000000]">{detailOrder?.code}</h3>
                  </div>
                  <div className="flex flex-row justify-between items-center">
                    <h3 className="text-base font-normal text-[#637381]">Ngày mua hàng:</h3>
                    <h3 className="text-base font-bold text-[#000000]">
                      {moment(detailOrder?.updateAt)?.isValid()
                        ? moment(detailOrder?.updateAt).format("DD/MM/YYYY")
                        : "--"}
                    </h3>
                  </div>
                  <div className="flex flex-row justify-between items-center">
